...
struct SidebarView: View {
    ...
    var body: some View {
        List(selection: filterSelection) {
            ...
            Section("Filter") {
                filterRow(.all, title: "Alle Bilder", systemImage: "photo.on.rectangle", count: app.groups.count)
                    .tag(PhotoFilter.all)
                filterRow(.unmarked, title: "Unmarkiert", systemImage: "circle.dashed", count: app.unmarkedCount)
                    .tag(PhotoFilter.unmarked)

                ForEach(1...9, id: \.self) { mark in
                    filterRow(.mark(mark),
                              title: "Markierung \(mark)",
                              dot: MarkStyle.color(for: mark),
    ...
    @ViewBuilder
    private func filterRow(_ filter: PhotoFilter, title: String,
                           systemImage: String? = nil, dot: Color? = nil, count: Int) -> some View {
        HStack(spacing: 8) {
            if let dot {
